...
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(
      ...
      downloadButton("downloadMap", "Download Map as HTML")
    ),
    mainPanel(
      fluidRow(
        column(
...
server <- function(input, output, session) {
  map_reactive <- reactive({
    leaflet() %>%
      ...
      addPolygons(data = province, layerId = ~PROV_CODE, 
                  ...
                  label = ~PROV_NAME,
                  group = "Province",
                  color = "black", weight = 3, fillColor = "transparent") %>%
      addLayersControl(
  ...
  observeEvent(input$map_shape_click, {
    ...
    clicked_layer <- click$id
    layer_type <- click$group
    
    if (layer_type == "Province") {
      districts_in_province <- district[district$PROV_CODE == clicked_layer, ]
      
      leafletProxy("map") %>%
        clearGroup("District") %>%
        clearGroup("Constituencies") %>%
        clearGroup("Wards") %>%
        fitBounds(click$bbox[1], click$bbox[2], click$bbox[3], click$bbox[4]) %>%
        addPolygons(data = districts_in_province, layerId = ~DIST_CODE, 
                    ...
                    label = ~DIST_NAME,
                    group = "District",
                    color = "#FFFF00", weight = 3, fillColor = "transparent")
    } else if (layer_type == "District") {
      constituencies_in_district <- constituencies[constituencies$DIST_CODE == clicked_layer, ]
      
      leafletProxy("map") %>%
        clearGroup("Constituencies") %>%
        clearGroup("Wards") %>%
        fitBounds(click$bbox[1], click$bbox[2], click$bbox[3], click$bbox[4]) %>%
        addPolygons(data = constituencies_in_district, layerId = ~CONST_CODE,
                    ...
                    color = "#008000", weight = 3, fillColor = "transparent")
    } else if (layer_type == "Constituencies") {
      wards_in_constituency <- wards[wards$CONST_CODE == clicked_layer, ]
      
      leafletProxy("map") %>%
        clearGroup("Wards") %>%
        fitBounds(click$bbox[1], click$bbox[2], click$bbox[3], click$bbox[4]) %>%
        addPolygons(data = wards_in_constituency, layerId = ~WARD_CODE,
                    ...
                    label = ~WARD_NAME,
                    group = "Wards",
                    color = "#FF0000", weight = 3, fillColor = "transparent")
    }
  })
  
  output$downloadMap <- downloadHandler(
    filename = function() {
      paste("leaflet_map-", Sys.Date(), ".html", sep = "")
    },
    content = function(file) {
      saveWidget(map_reactive(), file, selfcontained = TRUE)
    }
  )
  
  # Existing plotly outputs and download handlers
