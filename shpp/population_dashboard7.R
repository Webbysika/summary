...
ui <- fluidPage(
  sidebarLayout(
    ...
    mainPanel(
      fluidRow(
        column(width = 12, leafletOutput("map", height = 400))
      ),
      fluidRow(
        column(width = 12, h4("Population Graph"), selectInput("level", "Select Level:", choices = c("Province", "District", "Constituency")), plotlyOutput("popGraph"))
      )
    )
  )
)
...
server <- function(input, output, session) {
  ...
  output$map <- renderLeaflet({
    leaflet() %>%
      ...
      addPolygons(data = wards, layerId = ~WARD_CODE, popup = ~paste("Ward:", WARD_NAME, "<br>", "Total Population:", format(T_POP, big.mark = ","), "<br>", "Total Male:", format(T_Male, big.mark = ","), "<br>", "Total Female:", format(T_Female, big.mark = ","), "<br>", "Total Rural:", format(Tota_rural, big.mark = ","), "<br>", "Male Rural:", format(Male_Rural, big.mark = ","), "<br>", "Female Rural:", format(Femal_Rura, big.mark = ","), "<br>", "Total Urban:", format(Total_Urba, big.mark = ","), "<br>", "Male Urban:", format(Male_Urba, big.mark = ","), "<br>", "Female Urban:", format(Femal_Urb, big.mark = ",")), label = ~WARD_NAME, group = "Wards", color = "#C9C0DB", weight = 2, fillColor = "transparent") %>%
      addPolygons(data = constituencies, layerId = ~CONST_CODE, popup = ~paste("Constituency:", CONST_NAME, "<br>", "Total Population:", format(SUM_T_POP, big.mark = ","), "<br>", "Total Male:", format(SUM_T_Male, big.mark = ","), "<br>", "Total Female:", format(SUM_T_Fema, big.mark = ","), "<br>", "Total Rural:", format(SUM_Tota_r, big.mark = ","), "<br>", "Male Rural:", format(SUM_Male_R, big.mark = ","), "<br>", "Female Rural:", format(SUM_Femal_, big.mark = ","), "<br>", "Total Urban:", format(SUM_Total_, big.mark = ","), "<br>", "Male Urban:", format(SUM_Male_U, big.mark = ","), "<br>", "Female Urban:", format(SUM_Femal1, big.mark = ",")), label = ~CONST_NAME, group = "Constituencies", color = "#008000", weight = 2, fillColor = "transparent") %>%
      addPolygons(data = district, layerId = ~DIST_CODE, popup = ~paste("District:", DIST_NAME, "<br>", "Total Population:", format(SUM_T_POP, big.mark = ","), "<br>", "Total Male:", format(SUM_T_Male, big.mark = ","), "<br>", "Total Female:", format(SUM_T_Fema, big.mark = ","), "<br>", "Total Rural:", format(SUM_Tota_r, big.mark = ","), "<br>", "Male Rural:", format(SUM_Male_R, big.mark = ","), "<br>", "Female Rural:", format(SUM_Femal_, big.mark = ","), "<br>", "Total Urban:", format(SUM_Total_, big.mark = ","), "<br>", "Male Urban:", format(SUM_Male_U, big.mark = ","), "<br>", "Female Urban:", format(SUM_Femal1, big.mark = ",")), label = ~DIST_NAME, group = "District", color = "#809000", weight = 2, fillColor = "transparent") %>%
      addPolygons(data = province, layerId = ~PROV_CODE, popup = ~paste("Province:", PROV_NAME, "<br>", "Total Population:", format(SUM_T_POP, big.mark = ","), "<br>", "Total Male:", format(SUM_T_Male, big.mark = ","), "<br>", "Total Female:", format(SUM_T_Fema, big.mark = ","), "<br>", "Total Rural:", format(SUM_Tota_r, big.mark = ","), "<br>", "Male Rural:", format(SUM_Male_R, big.mark = ","), "<br>", "Female Rural:", format(SUM_Femal_, big.mark = ","), "<br>", "Total Urban:", format(SUM_Total_, big.mark = ","), "<br>", "Male Urban:", format(SUM_Male_U, big.mark = ","), "<br>", "Female Urban:", format(SUM_Femal1, big.mark = ",")), label = ~PROV_NAME, group = "Province", color = "black", weight = 2, fillColor = "transparent") %>%
      addLayersControl(overlayGroups = c("Province", "District", "Constituencies", "Wards"), options = layersControlOptions(collapsed = FALSE))
  })
  
  # Gender population graph rendering
  output$genderPopGraph <- renderPlotly({
    province_data <- province %>% arrange(desc(SUM_T_POP)) %>% mutate(Female = SUM_T_Fema, Male = SUM_T_Male)
    ...
    ggplotly(p, tooltip = c("y")) %>% layout(hovermode = "x unified") %>% event_register("plotly_click")
  })
  
  # Region population graph rendering
  output$regionPopGraph <- renderPlotly({
    province_data <- province %>% arrange(desc(SUM_T_POP)) %>% mutate(Rural = SUM_Tota_r, Urban = SUM_Total_)
    p <- ggplot(province_data, aes(x = reorder(PROV_NAME, -SUM_T_POP))) +
  ...
  # Dynamic population graph rendering based on user selection
